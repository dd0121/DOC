...
def filter_local_features(opt,
    ...
    source_valid_char_label = source_pred_class.reshape(-1, ).index_select(0, source_valid_char_index)
    ...
    target_valid_char_label = target_pred_class.reshape(-1, ).index_select(0, target_valid_char_index)

    return source_valid_char_feature, source_valid_char_label, source_valid_char_index, target_valid_char_feature, target_valid_char_label, target_valid_char_index
...
class trainer(object):
    ...
    def _optimizer(self, opt):
        ...
        # setup optimizer
        # self.center_optimizer = torch.optim.SGD([*self.model.parameters(),*self.center_criterion.parameters()], lr=opt.lr)

        if opt.optimizer.lower() == 'sgd':
            self.optimizer = optim.SGD([*self.model.parameters(), *self.center_criterion.parameters()], lr=opt.lr, momentum=opt.momentum,
            ...
            self.optimizer = optim.Adadelta([*filtered_parameters, *self.center_criterion.parameters()], lr=0.1 * opt.lr, rho=opt.rho,
                                            eps=opt.eps)
    ...
    def build_model(self, opt):
        ...
        self.center_criterion = CenterLoss(num_classes=38, feat_dim=256, use_gpu=True)
        self.local_criterion= torch.nn.CrossEntropyLoss().to(device)
    ...
    def train(self, opt):
        ...
        for step in range(start_iter, opt.num_iter + 1):
            ...
            self.center_criterion.zero_grad()
            ...
            src_local_feature1, src_local_label, src_local_index, tar_local_feature1, tar_local_label, tar_local_index= filter_local_features(opt, src_local_feature,
                                                                         src_preds,
                                                                         tar_local_feature,
                                                                         tar_preds)

            src_local_gt = target.reshape(-1, ).index_select(0, src_local_index)
            src_local_preds = src_preds.reshape(-1, src_preds.size(2)).index_select(0, src_local_index)
            if len(src_local_preds)>0:
                local_l = self.local_criterion(src_local_preds,src_local_gt)
            else:
                local_l = 0


            # src_local_label_onehot = torch.zeros(src_local_label.size(0), src_preds.size(0)).scatter_(1, src_local_label[:,None], 1)
            # tar_local_label_onehot = torch.zeros(tar_local_label.size(0), tar_preds.size(0)).scatter_(1, tar_local_label[:,None], 1)
            local_feature = torch.cat([src_local_feature1, tar_local_feature1], dim=0)
            local_label = torch.cat([src_local_gt, tar_local_label], dim=0)
            local_cls_loss = self.center_criterion(local_feature, local_label)
            ...
            # Add domain loss
            loss = src_cls_loss.mean() + omega * (d_img_loss.mean()+d_inst_loss.mean()) + 0.0001 * local_cls_loss + local_l
            loss_avg.add(loss)
            cls_loss_avg.add(src_cls_loss)
            sim_loss_avg.add(d_img_loss)
            center_avg.add(local_cls_loss)
            ...
            if step % opt.valInterval == 0:
                ...
                print(
                    f'[{step}/{opt.num_iter}] Loss: {loss_avg.val():0.5f} CLS_Loss: {cls_loss_avg.val():0.5f} SIMI_Loss: {sim_loss_avg.val():0.5f} CENTER_Loss: {center_avg.val():0.5f} elapsed_time: {elapsed_time:0.5f}')
                # for log
                with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a') as log:
                    ...
                    if current_accuracy > best_accuracy:
                        ...
                        save_name = f'./saved_models/{opt.experiment_name}/iter_{step+1}_best_accuracy.pth'
                        self.save(opt, save_name)
                    if current_norm_ED < best_norm_ED:
                        best_norm_ED = current_norm_ED
                        save_name = f'./saved_models/{opt.experiment_name}/iter_{step+1}_best_norm_ED.pth'
                        self.save(opt, save_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_iter', type=int, default=232000,
    ...
    parser.add_argument('--tar_select_data', type=str, default='',
